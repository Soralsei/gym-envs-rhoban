...
class TurningRadiusReward(RewardWrapper):
    ...
    def reward(self, reward: float) -> float:  # type: ignore
        ...
        dist_left = np.linalg.norm(
            goal_in_robot_frame - np.array([0, self.min_turning_radius])
        )
        dist_right = np.linalg.norm(
            goal_in_robot_frame - np.array([0, -self.min_turning_radius])
        )
        penalty = 0.0
        if dist_left < (self.min_turning_radius - self.env.unwrapped.distance_threshold) or dist_right < (self.min_turning_radius - self.env.unwrapped.distance_threshold):
