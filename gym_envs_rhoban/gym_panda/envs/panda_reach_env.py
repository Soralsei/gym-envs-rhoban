...
class SimplePandaReachEnv(gym.Env):
    ...
    def step(self, action):
        ...
        error = np.linalg.norm(obs[0:3] - self.goal, axis=-1)
    ...
    def _set_action(self, action):
        ...
        ee_state = p.getLinkState(self.panda_id, self.arm_eef_index)
        cur_position = np.array(ee_state[0])
        cur_orient = np.array(ee_state[1])

        new_pos = cur_position + np.array(action[:3]) * self.max_vel * self.dt
        ...
        jointPoses = p.calculateInverseKinematics(
            ...
            new_pos,
            cur_orient,
            maxNumIterations=self.n_substeps,
        )
        for i in range(len(jointPoses)):
            p.setJointMotorControl2(
                self.panda_id, i, p.POSITION_CONTROL, jointPoses[i], force=5 * 240.0
            )

    def _get_obs(self):
        ee_state = p.getLinkState(
            self.panda_id, self.arm_eef_index, computeLinkVelocity=True
        )
        grip_pos = np.array(
            ee_state[0]
        )  # gripper cartesian world position = 3 float (x,y,z) = achieved goal
        grip_velp = np.array(
            ee_state[6]
        )  # gripper cartesian world velocity = 3 float (vx, vy, vz)
        # self.goal = desired goal, 3 float (xg, yg, zg)

        # observation = 9 float
        obs = np.concatenate((grip_pos, grip_velp, self.goal)).astype(np.float32)
